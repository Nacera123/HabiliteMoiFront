...
@Component({
  ...
  templateUrl: './lien-pole.html',
  styleUrl: './lien-pole.css',
})
export class LienPole implements OnInit{
  ...
  get polesUniques(): Pole[] {
    const map = new Map<number, Pole>();
    this.lienPole.forEach(lp => {
      if (lp.pole?.id && !map.has(lp.pole.id)) {
        map.set(lp.pole.id, lp.pole);
      }
    });
